    ...
    const ofMesh& sphereMesh, const ofShader& shader)
{
    ...
    neckMesh->localTransform = translate(vec3(0, 1.1f, 0)) * scale(vec3(0.5f));
    ...
    headMesh->localTransform = translate(vec3(0, 1.75f, 0)) * scale(vec3(0.5f));
    ...
    // Create joint node animation (left leg)
    cubeMeshNode->childNodes.emplace_back(new ComplexAnimationNode(-1.0f, vec3(1, 0, 0), 0.5f));
