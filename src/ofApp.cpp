...
void ofApp::setup()
{
    ...
    // Load shield mesh
    shieldMesh.load("models/shield.ply");
    staffMesh.load("models/staff2.ply");
    jarMesh.load("models/jar2.ply");
...
void ofApp::update()
{
    ...
    staffPosition = vec3(10282.8 + sin(time), 754.475, 8669);
...
void ofApp::drawScene(CameraMatrices& camMatrices, int reflection)
{
    ...
    drawMesh(camMatrices, dirLight, vec3(0.0),
        directionalLightShader2, jarMesh,
        translate(vec3(shieldPosition)));
    drawMesh(camMatrices, dirLight, vec3(0.0),
        directionalLightShader2, staffMesh,
